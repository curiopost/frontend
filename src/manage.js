...
export default function Manage() {
    ...
    useEffect(() => {
        const setPostData = async() => {
            const getPostdata = await fetch(urls.backend+`/api/read/post?id=${id}`)
            const data = await getPostdata.json()
            if(data.success && data.raw_data.user_id === udata.raw_data._id) {
                setForm([
                    <>
                        <div className="card">
                            <div className="card-body">
                                <h5 className="card-title">Managing a Post: '{data.raw_data.title}'</h5>
                                <p className="card-text">Make sure to click on "Save Changes" to save your changes once your done.</p>
                                <div className="input-group mb-4 p-1">
                                    <span className="input-group-text" data-toggle="tooltip" data-placement="top" title="Post Title">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" class="bi bi-chat-right-dots" viewBox="0 0 16 16"><path d="M2 1a1 1 0 0 0-1 1v8a1 1 0 0 0 1 1h9.586a2 2 0 0 1 1.414.586l2 2V2a1 1 0 0 0-1-1H2zm12-1a2 2 0 0 1 2 2v12.793a.5.5 0 0 1-.854.353l-2.853-2.853a1 1 0 0 0-.707-.293H2a2 2 0 0 1-2-2V2a2 2 0 0 1 2-2h12z"></path><path d="M5 6a1 1 0 1 1-2 0 1 1 0 0 1 2 0zm4 0a1 1 0 1 1-2 0 1 1 0 0 1 2 0zm4 0a1 1 0 1 1-2 0 1 1 0 0 1 2 0z"></path>
                                    </svg>
                                    </span>
                                    <input className="form-control" id="update-post-title" type="text" placeholder="Post Title" defaultValue={data.raw_data.title}></input>
                                    </div>
                                    <div className="input-group mb-4 p-1">
                                        <span className="input-group-text" data-toggle="tooltop" data-placement="top" title="Post Description"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" class="bi bi-blockquote-left" viewBox="0 0 16 16"><path d="M2.5 3a.5.5 0 0 0 0 1h11a.5.5 0 0 0 0-1h-11zm5 3a.5.5 0 0 0 0 1h6a.5.5 0 0 0 0-1h-6zm0 3a.5.5 0 0 0 0 1h6a.5.5 0 0 0 0-1h-6zm-5 3a.5.5 0 0 0 0 1h11a.5.5 0 0 0 0-1h-11zm.79-5.373c.112-.078.26-.17.444-.275L3.524 6c-.122.074-.272.17-.452.287-.18.117-.35.26-.51.428a2.425 2.425 0 0 0-.398.562c-.11.207-.164.438-.164.692 0 .36.072.65.217.873.144.219.385.328.72.328.215 0 .383-.07.504-.211a.697.697 0 0 0 .188-.463c0-.23-.07-.404-.211-.521-.137-.121-.326-.182-.568-.182h-.282c.024-.203.065-.37.123-.498a1.38 1.38 0 0 1 .252-.37 1.94 1.94 0 0 1 .346-.298zm2.167 0c.113-.078.262-.17.445-.275L5.692 6c-.122.074-.272.17-.452.287-.18.117-.35.26-.51.428a2.425 2.425 0 0 0-.398.562c-.11.207-.164.438-.164.692 0 .36.072.65.217.873.144.219.385.328.72.328.215 0 .383-.07.504-.211a.697.697 0 0 0 .188-.463c0-.23-.07-.404-.211-.521-.137-.121-.326-.182-.568-.182h-.282a1.75 1.75 0 0 1 .118-.492c.058-.13.144-.254.257-.375a1.94 1.94 0 0 1 .346-.3z"></path>
                                        </svg></span>
                                        <textarea rows="3" id="update-post-description" className="form-control" placeholder="Post Description" defaultValue={data.raw_data.content}></textarea>
                                        </div>
                                        <button className="btn btn-success w-100" style={{"marginBottom": "1%"}} onClick={saveChanges} id="savechangesbtn">Save Changes</button>
                                 
                                        <button className="btn btn-danger w-100" data-bs-toggle="modal" data-bs-target="#deletePostModal">Delete Post</button>
                                    </div>
                        </div>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="deletePostModallabel" >Danger! Deleting a Post!</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
 <p>Please not that once you click the delete button there is not turning back, and your post will be gone forever, along with all the replies and likes.

 </p>
      </div>
      <div class="modal-footer">
        
        <button className="btn btn-danger w-100" id="deletebtn" onClick={deleteData}>Delete this Post Forever</button>
      </div>
    </div>
  </div>
</div>
        ...
        const setQuestionData = async() => {
            const getQuestionData = await fetch(urls.backend+`/api/read/question?id=${id}`)
            const data = await getQuestionData.json()
            if(data.success && data.raw_data.user_id === udata.raw_data._id) {
                setForm([
                <>
                <div className="card">
                    <div className="card-body">
                        <h5 className="card-title">Managing a Question: '{data.raw_data.title}'</h5>
                        <p className="card-text">Make sure to click on "Save Changes" to save your changes once your done.</p>
                        <div className="input-group mb-4 p-1">
                            <span className="input-group-text" data-toggle="tooltip" data-placement="top" title="Question Title">
                            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" class="bi bi-chat-right-dots" viewBox="0 0 16 16"><path d="M2 1a1 1 0 0 0-1 1v8a1 1 0 0 0 1 1h9.586a2 2 0 0 1 1.414.586l2 2V2a1 1 0 0 0-1-1H2zm12-1a2 2 0 0 1 2 2v12.793a.5.5 0 0 1-.854.353l-2.853-2.853a1 1 0 0 0-.707-.293H2a2 2 0 0 1-2-2V2a2 2 0 0 1 2-2h12z"></path><path d="M5 6a1 1 0 1 1-2 0 1 1 0 0 1 2 0zm4 0a1 1 0 1 1-2 0 1 1 0 0 1 2 0zm4 0a1 1 0 1 1-2 0 1 1 0 0 1 2 0z"></path>
                            </svg>
                            </span>
                            <input className="form-control" id="update-post-title" type="text" placeholder="Question Title" defaultValue={data.raw_data.title}></input>
                            </div>
                            <div className="input-group mb-4 p-1">
                                <span className="input-group-text" data-toggle="tooltop" data-placement="top" title="Question Description"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" class="bi bi-blockquote-left" viewBox="0 0 16 16"><path d="M2.5 3a.5.5 0 0 0 0 1h11a.5.5 0 0 0 0-1h-11zm5 3a.5.5 0 0 0 0 1h6a.5.5 0 0 0 0-1h-6zm0 3a.5.5 0 0 0 0 1h6a.5.5 0 0 0 0-1h-6zm-5 3a.5.5 0 0 0 0 1h11a.5.5 0 0 0 0-1h-11zm.79-5.373c.112-.078.26-.17.444-.275L3.524 6c-.122.074-.272.17-.452.287-.18.117-.35.26-.51.428a2.425 2.425 0 0 0-.398.562c-.11.207-.164.438-.164.692 0 .36.072.65.217.873.144.219.385.328.72.328.215 0 .383-.07.504-.211a.697.697 0 0 0 .188-.463c0-.23-.07-.404-.211-.521-.137-.121-.326-.182-.568-.182h-.282c.024-.203.065-.37.123-.498a1.38 1.38 0 0 1 .252-.37 1.94 1.94 0 0 1 .346-.298zm2.167 0c.113-.078.262-.17.445-.275L5.692 6c-.122.074-.272.17-.452.287-.18.117-.35.26-.51.428a2.425 2.425 0 0 0-.398.562c-.11.207-.164.438-.164.692 0 .36.072.65.217.873.144.219.385.328.72.328.215 0 .383-.07.504-.211a.697.697 0 0 0 .188-.463c0-.23-.07-.404-.211-.521-.137-.121-.326-.182-.568-.182h-.282a1.75 1.75 0 0 1 .118-.492c.058-.13.144-.254.257-.375a1.94 1.94 0 0 1 .346-.3z"></path>
                                </svg></span>
                                <textarea rows="3" id="update-post-description" className="form-control" placeholder="Question Description" defaultValue={data.raw_data.content}></textarea>
                                </div>
                                <button className="btn btn-success w-100" style={{"marginBottom": "1%"}} onClick={saveChanges} id="savechangesbtn">Save Changes</button>
                         
                                <button className="btn btn-danger w-100" data-bs-toggle="modal" data-bs-target="#deletePostModal">Delete Question</button>
                            </div>
                </div>
...
<div class="modal-content">
<div class="modal-header">
<h5 class="modal-title" id="deletePostModallabel" >Danger! Deleting a Question!</h5>
<button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
</div>
<div class="modal-body">
<p>Please not that once you click the delete button there is not turning back, and your question will be gone forever, along with all the replies and likes.

</p>
</div>
<div class="modal-footer">

<button className="btn btn-danger w-100" id="deletebtn" onClick={deleteData}>Delete this Question Forever</button>
</div>
</div>
</div>
</div>
        ...
        const setReplyData = async() => { 
            const getReply = await fetch(urls.backend+`/api/read/reply?id=${id}`)
            const data = await getReply.json()
            if(data.success && data.raw_data.user_id === udata.raw_data._id) {
                setForm([
                    <>
                    <div className="card">
                        <div className="card-body">
                            <h5 className="card-title">Managing a Reply</h5>
                            <p className="card-text">Make sure to click on "Save Changes" to save your changes once your done.</p>
                           
                                <div className="input-group mb-4 p-1">
                                    <span className="input-group-text" data-toggle="tooltop" data-placement="top" title="Reply Description"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" class="bi bi-blockquote-left" viewBox="0 0 16 16"><path d="M2.5 3a.5.5 0 0 0 0 1h11a.5.5 0 0 0 0-1h-11zm5 3a.5.5 0 0 0 0 1h6a.5.5 0 0 0 0-1h-6zm0 3a.5.5 0 0 0 0 1h6a.5.5 0 0 0 0-1h-6zm-5 3a.5.5 0 0 0 0 1h11a.5.5 0 0 0 0-1h-11zm.79-5.373c.112-.078.26-.17.444-.275L3.524 6c-.122.074-.272.17-.452.287-.18.117-.35.26-.51.428a2.425 2.425 0 0 0-.398.562c-.11.207-.164.438-.164.692 0 .36.072.65.217.873.144.219.385.328.72.328.215 0 .383-.07.504-.211a.697.697 0 0 0 .188-.463c0-.23-.07-.404-.211-.521-.137-.121-.326-.182-.568-.182h-.282c.024-.203.065-.37.123-.498a1.38 1.38 0 0 1 .252-.37 1.94 1.94 0 0 1 .346-.298zm2.167 0c.113-.078.262-.17.445-.275L5.692 6c-.122.074-.272.17-.452.287-.18.117-.35.26-.51.428a2.425 2.425 0 0 0-.398.562c-.11.207-.164.438-.164.692 0 .36.072.65.217.873.144.219.385.328.72.328.215 0 .383-.07.504-.211a.697.697 0 0 0 .188-.463c0-.23-.07-.404-.211-.521-.137-.121-.326-.182-.568-.182h-.282a1.75 1.75 0 0 1 .118-.492c.058-.13.144-.254.257-.375a1.94 1.94 0 0 1 .346-.3z"></path>
                                    </svg></span>
                                    <textarea rows="3" id="update-post-description" className="form-control" placeholder="Reply Description" defaultValue={data.raw_data.content}></textarea>
                                    </div>
                                    <button className="btn btn-success w-100" style={{"marginBottom": "1%"}} onClick={saveChanges} id="savechangesbtn">Save Changes</button>
                             
                                    <button className="btn btn-danger w-100" data-bs-toggle="modal" data-bs-target="#deletePostModal">Delete Reply</button>
                                </div>
                    </div>
    ...
    <div class="modal-content">
    <div class="modal-header">
    <h5 class="modal-title" id="deletePostModallabel" >Danger! Deleting a Reply!</h5>
    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
    </div>
    <div class="modal-body">
    <p>Please not that once you click the delete button there is not turning back, and your reply will be gone forever, along with all the likes.
    
    </p>
    </div>
    <div class="modal-footer">
    
    <button className="btn btn-danger w-100" id="deletebtn" onClick={deleteData}>Delete this Reply Forever</button>
    </div>
    </div>
    </div>
    </div>
    ...
    const saveChanges = async() => {
      ...
      if(type === "posts" || type === "questions") {
        buffer(document.getElementById('savechangesbtn'), "Saving Changes...")
        const title = document.getElementById('update-post-title').value 
        const content = document.getElementById('update-post-description').value 
        const topics = []
        let r = content.split(' ')
        ...
        const sendUpdateRequest = await fetch(urls.backend+`/api/update/post?id=${id}`, {
            method: 'PATCH',
            headers: {
            "Content-Type": "application/json",
            "token": token
        ...
        if(data.success) {
            unbuffer(document.getElementById('savechangesbtn'), "Save Changes")
        ...
        const sendUpdateRequest = await fetch(urls.backend+`/api/update/reply?id=${id}`, {
            method: 'PATCH',
            headers: {
            "Content-Type": "application/json",
            "token": token
    ...
    const deleteData = async() => {
        const token = window.localStorage.getItem("token")
        if(type === "posts" || type === "questions") {
            buffer(document.getElementById('deletebtn'), "Deleting...")
            const sendDelReq = await fetch(urls.backend+`/api/delete/post?id=${id}`, {
                method: 'DELETE',
                headers: {
                "Content-Type": "application/json",
                "token": token
            ...
            const data = await sendDelReq.json()
            if(data.success) {

                buffer(document.getElementById('deletebtn'), "Successfully deleted data, reloading...")
                setTimeout(() => {
                window.location.href = `/${type}/${id}/manage`
            }, 2000)
            return;
            } else {
                unbuffer(document.getElementById('deletebtn'), "Retry Deleting?")
            ...
            buffer(document.getElementById('deletebtn'), "Deleting...")
            const sendDelReq = await fetch(urls.backend+`/api/delete/reply?id=${id}`, {
                method: 'DELETE',
                headers: {
                "Content-Type": "application/json",
                "token": token
