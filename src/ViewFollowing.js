...
export default function ViewFollowing(props) {
    ...
    const [following, setFollowing] = useState([<p className="text-center">Loading...</p>])

    useEffect(() => {
const setFollowewingF = async() => {
    const getData = await fetch(urls.backend+`/api/read/following?username=${username}`, {
        method: 'GET'
    })
    ...
    if(data.following.length < 1) {
        setFollowing([<p className="text-center">This user is not following anyone : (</p>])
    }
    data.following.forEach(f => {

            setFollowing(following => [...following, <><div className="card" style={{"marginBottom": "1%"}}>
                <div className="card-body d-flex">
                    <img className="rounded" height="50" width="50" src={f.avatar_url ? f.avatar_url: "https://curiopost.live/logo.png"}></img>
                    <Link to={"/u/"+f.username} className="text-decoration-none text-dark"><h5 className="text-muted" style={{"marginTop": "10px", "marginLeft": "5px"}}>{f.name} (@{f.username}) </h5>
                  
                   </Link>  </div></div></>])

        })
    } else {
        setFollowing([<p className="text-center">Unexpected error occured : (</p>])
    }
}
...
setFollowewingF()
    ...
    return (<>
        ...
        {islgin ? <nav className="navbar navbar-expand-lg bg-dark navbar-dark sticky-top">
        ...
        <div class="col-lg-8 offset-lg-2" style={{ "marginTop": "7%" }}>
            <Link to={"/u/"+username} className="text-decoration-none text-dark"><h5>{username} is currently following</h5></Link>
            {following}
        </div>
        </div>
