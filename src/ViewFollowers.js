...
export default function ViewFollowers(props) {
    ...
    const [followers, setFollowers] = useState([<p className="text-center">Loading...</p>])

    useEffect(() => {
const setFollowersF = async() => {
    const getData = await fetch(urls.backend+`/api/read/followers?username=${username}`, {
        method: 'GET'
    })
    ...
    if(data.followers.length < 1) {
        setFollowers([<p className="text-center">This user has no followers : (</p>])
    }
        data.followers.forEach(f => {

            setFollowers(followers => [...followers, <><div className="card" style={{"marginBottom": "1%"}}>
        ...
        setFollowers([<p className="text-center">Unexpected error occured : (</p>])
    }
}
...
setFollowersF()
    ...
    return (<>
        ...
        <div class="col-lg-8 offset-lg-2" style={{ "marginTop": "7%" }}>
            <Link to={"/u/"+username} className="text-decoration-none text-dark"><h5>{username}'s followers</h5></Link>
            {followers}
        </div>
        </div>
