...
export default function loginPage() {
    ...
    const login = async() => {
        ...
        const resp = await fetch(urls.backend+'/api/auth/login', {
            method: 'POST',
            headers: {
...
<input type="password" className="form-control"  placeholder="Password"  aria-label="Email" aria-describedby="b2" id="password"/>
            ...
            <button className=" btn btn-success w-100 text-center" id="loginbtn" style={{"marginBottom": "2%", }} onClick={login}>Login</button>      
            <Link to="/register" className="float-start text-muted">Don't have an account? Create One!</Link>
